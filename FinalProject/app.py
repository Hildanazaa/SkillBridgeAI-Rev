...
with col_left:
    ...
    if st.session_state.show_recommendations and st.session_state.recommendations:
        ...
        for i, rec in enumerate(st.session_state.recommendations):
            ...
            if st.button(f"Pilih  {rec['title']}", key=f"rec_{rec['title']}"):
                st.session_state.selected_career = rec["title"]
                with st.spinner("Mengevaluasi kesiapan..."):
                    missing = rec.get("missing_skills", [])
                    st.session_state.analysis = get_analysis(st.session_state.user_skills_text, rec["title"])
                    st.session_state.roadmap = get_roadmap(rec["title"], missing)
...
with col_right:
    ...
    if st.button("Evaluasi Kesiapan"):
        if not recs:
            ...
            with st.spinner("Mengevaluasi..."):
                missing = next((r["missing_skills"] for r in recs if r["title"] == selected), [])
                st.session_state.selected_career = selected
                st.session_state.analysis = get_analysis(st.session_state.user_skills_text, selected)
                st.session_state.roadmap = get_roadmap(selected, missing)
                st.session_state.show_evaluation = True
            st.rerun()
    ...
    if st.session_state.show_evaluation and st.session_state.analysis:
        ...
        st.markdown('<div style="background:#ffffff; border:1px solid #e5e7eb; border-radius:12px; padding:16px; margin-bottom:12px;">' + miss_label + '<div>' + badges + '</div></div>', unsafe_allow_html=True)
                    ...
                    for i in range(0, len(days), 2):
                        ...
                        for j, col in enumerate(cols):
                            if i + j < len(days):
                                ...
                                resource_html = ""
                                if day["resource"] != "-":
                                    resource_title = day["resource"]
                                    resource_url = day.get("resource_url")

                                    if resource_url:
                                        resource_html = (
                                            '<p style="font-size:11px; color:#16a34a; font-weight:500; margin:4px 0 0 0;">'
                                            '&#128218; Resource: '
                                            '<a href="' + resource_url + '" target="_blank" rel="noopener noreferrer" '
                                            'style="color:#a5f3fc; text-decoration:underline;">' + resource_title + '</a>'
                                            '</p>'
                                        )
                                    elif resource_title.startswith("http"):
                                        resource_html = (
                                            '<p style="font-size:11px; color:#16a34a; font-weight:500; margin:4px 0 0 0;">'
                                            '&#128218; Resource: '
                                            '<a href="' + resource_title + '" target="_blank" rel="noopener noreferrer" '
                                            'style="color:#a5f3fc; text-decoration:underline;">' + resource_title + '</a>'
                                            '</p>'
                                        )
                                    else:
                                        resource_html = '<p style="font-size:11px; color:#16a34a; font-weight:500; margin:4px 0 0 0;">&#128218; Resource: ' + resource_title + '</p>'

                                with col:
                                    st.markdown(
                                        '<div style="background:#1e3a5f; border-radius:10px; padding:14px 16px; margin-bottom:10px; min-height:120px;">'
                                        '<p style="font-size:14px; font-weight:600; color:#ffffff; margin:0 0 4px 0;">Hari ' + str(day['day']) + ' — ' + day['topic'] + '</p>'
                                        '<p style="font-size:13px; color:#cbd5e1; margin:0;">' + day['detail'] + '</p>'
                                        + resource_html +
                                        '</div>',
                                        unsafe_allow_html=True
                                    )
